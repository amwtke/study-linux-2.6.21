...
void sock_init_data(struct socket *sock, struct sock *sk)
{
	...
	sk->sk_state_change	=	sock_def_wakeup; // socket状态改变时回调
	sk->sk_data_ready	=	sock_def_readable; //数据可读时回调
	sk->sk_write_space	=	sock_def_write_space; //写数据空间可用时回调
	sk->sk_error_report	=	sock_def_error_report; // 发生错误时回调
